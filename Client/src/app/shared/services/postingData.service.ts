    ...
    isDayInfoValid(dayInfo: DayInfo): boolean {
        if ((dayInfo.TimeOfStart >= dayInfo.TimeOfFinish)
            ...
            || ((dayInfo.TimeOfStart2 >= dayInfo.TimeOfFinish2)
                && ((dayInfo.TimeOfStart2 !== PublicConstants.DEFAULT_BLANK_VALUE_FOR_TIME_AND_DATE)
                    || (dayInfo.TimeOfFinish2 !== PublicConstants.DEFAULT_BLANK_VALUE_FOR_TIME_AND_DATE)))
        ...
        else if (((dayInfo.TimeOfFinish2 > 1440) && (dayInfo.TimeOfFinish2 !== PublicConstants.DEFAULT_BLANK_VALUE_FOR_TIME_AND_DATE))
            || ((dayInfo.TimeOfStart2 >= 1440) && (dayInfo.TimeOfStart2 !== PublicConstants.DEFAULT_BLANK_VALUE_FOR_TIME_AND_DATE))) {
            return false;
        }
        else {
            return true;
